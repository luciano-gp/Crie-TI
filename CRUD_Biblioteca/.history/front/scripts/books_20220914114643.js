    ...
    .then((response) => {
      if (response.status === 200) {
        ...
        data.forEach(element => {
          ...
          trHTML += '<td>' + element.Publisher.City.name + '</td>';
          trHTML += '<td>' + element.Publisher.City.State.name + '</td>';
          trHTML += '<td><button type="button" class="btn btn-outline-light" onclick="showBookEditBox(' + element.id + ')">Edit</button>';
...
const bookEdit = () => {
  ...
  const publisher = document.getElementById("mySelect").value;
  const categorie = document.getElementById("mySelect2").value;
  console.log(id, title, author, year, pages, format, publisher, categorie);
        ...
        if (Number(option) === 1) {
          data.forEach((dat) => {
            if (dat.title.match(input)) {
              ...
              trHTML += '<td><button type="button" class="btn btn-outline-light" onclick="showBookEditBox(' + dat.id + ')">Edit</button>';
              trHTML += '<button type="button" class="btn btn-outline-danger" onclick="bookDelete(' + dat.id + ')">Del</button></td>';
              trHTML += "</tr>";
            }
          });
        } else {
